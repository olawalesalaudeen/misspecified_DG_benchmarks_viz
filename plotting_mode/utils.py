...
COLOR_MAP = {
    0: "blue", 1: "red", 2: "green", 3: "orange", 4: "purple", 5: "brown",
    6: "pink", 7: "gray", 8: "olive", 9: "cyan", 10: "magenta", 11: "yellow",
    12: "black", 13: "darkblue", 14: "darkred", 15: "darkgreen", 16: "darkorange",
    17: "darkpurple", 18: "darkbrown", 19: "darkpink", 20: "darkgray", 21: "darkolive",
    22: "darkcyan", 23: "darkmagenta", 24: "darkyellow"
}
...
def scatter_plot(df, env, scaling="Probit", do_domain=True, legend_change=False, show_linear_fits=True, colors="blue", fitcolor="black"):
    """
    ...
    if not legend_change:
        if show_linear_fits and do_domain:
            ...
            for unique_env in unique_envs:
                ...
                slope, intercept, pearson_corr, _, _ = linregress(scaled_xs[env_mask].flatten(), scaled_ys[env_mask].flatten())
        ...
        model_arch_to_color_idx = zip(range(len(unique_model_archs)), unique_model_archs)
        model_arch_color_dict = {model_arch: COLOR_MAP[idx] for idx, model_arch in model_arch_to_color_idx}
        
        for model_arch in unique_model_archs:
            model_arch_mask = df['model_arch'] == model_arch
            slope, intercept, pearson_corr, _, _ = linregress(scaled_xs[model_arch_mask].flatten(), scaled_ys[model_arch_mask].flatten())
            sample_pts = rescale(np.arange(0.0, 1.0, 0.01), scaling)
            traces.append(
                go.Scatter(
                    mode="lines",
    ...
    slope, intercept, pearson_corr, _, _ = linregress(scaled_xs.flatten(), scaled_ys.flatten())
    sample_pts = rescale(np.arange(0.0, 1.0, 0.01), scaling)
    traces.append(
        go.Scatter(
            mode="lines",
...
def display_env_data(df, env, scaling="Probit"):
    ...
    for unique_env in unique_envs:
        env_mask = env == unique_env
        slope, intercept, pearson_corr, p_value, std_err = linregress(scaled_xs[env_mask].flatten(), scaled_ys[env_mask].flatten())
        env_data.append({
...
def plot(df, scaling="Probit", show_linear_fits=True, metric="Accuracy", all_shown=False, legend_change=False):
    """
    Generate an interactive scatter plot.
    """
    title = f"Accuracy on the Line for Test Domain"
    fig = make_subplots(
        rows=1, cols=1, subplot_titles=((f"{title} ({scaling})"), ),
    )
    fig.update_annotations(font=dict(color="black"))
    traces = []

    # Obtain color mapping for model architectures and environments
    model_arch_to_color_idx = zip(range(len(df['model_arch'].unique())), df['model_arch'].unique())
    model_arch_color_dict = {model_arch: COLOR_MAP[idx] for idx, model_arch in model_arch_to_color_idx}
    model_arch_color_mapping = df['model_arch'].map(model_arch_color_dict).values
    ...
    plot_traces, env_df = scatter_plot(
        ...
        colors=(env_color_mapping if not legend_change else model_arch_color_mapping),
    )
