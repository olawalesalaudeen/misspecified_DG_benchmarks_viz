...
def load_records(path):
    ...
    for root, _, files in os.walk(path):
        json_files = [
            f for f in files if f.endswith(".json") or f.endswith(".jsonl")
        ]
...
def load_data():
    ...
    with ProcessPoolExecutor() as executor:
        datasets = [
            ...
            if d != ".git"
            and d != "ood_bench_figures"
            and os.path.isdir(os.path.join(results_dir, d))
        ]
        results = executor.map(
            process_dataset, [results_dir] * len(datasets), datasets
        )
...
def load_parsed_data():
    ...
    for dataset in os.listdir(results_dir):
        ...
        if os.path.isdir(dataset_path):
            ...
            if csv_files:
                df_list = [
                    pd.read_csv(os.path.join(dataset_path, csv_file))
...
def run_real_data_plotting():
    ...
    selected_dataset = st.sidebar.selectbox(
        "Dataset", sorted(list(records_df.keys()))
    )
    ...
    dataset_df["model_arch"] = dataset_df["model_hparams"].apply(
        lambda x: extract_metric(x).get("model_arch", "None")
    )
    dataset_df["transfer"] = dataset_df["model_hparams"].apply(
        lambda x: extract_metric(x).get("transfer", "None")
    )

    test_envs = dataset_df.test_env.values.tolist()
    test_env = st.sidebar.selectbox(
        "Left Out Test Domain #", sorted(set(test_envs))
    )

    # Filtered View on Environments
    filtered_envs = st.sidebar.multiselect(
        "Filtered Environment #",
        sorted(set(test_envs)),
        sorted(set(test_envs)),
    )

    # Filter Based Model Arch
    filtered_model_arch = st.sidebar.multiselect(
        "Filtered Model Architecture",
        sorted(set(dataset_df["model_arch"].values)),
        sorted(set(dataset_df["model_arch"].values)),
    )

    # Filter Based Transfer Learning
    filtered_transfer = st.sidebar.toggle(
        "Show Models with Transfer Learning", value=True
    )
    ...
    legend_change = st.sidebar.toggle(
        "Change Plot Legend to Model Architecture", value=False
    )
    ...
    if dataset_df.shape[0]:
        ...
        if is_dark_mode:
            # Update all text elements to be light colored
            full_plot.update_layout(
                paper_bgcolor="#0E1117",
                plot_bgcolor="#0E1117",
                font_color="#E6E6E6",  # Light gray for all text
                legend=dict(
                ...
                title=dict(
                    text=full_plot.layout.title.text,  # Keep existing title text
                    font=dict(color="#E6E6E6"),
                ),
            )

            # Update subplot titles to be light colored
            for annotation in full_plot.layout.annotations:
                annotation.update(font=dict(color="#E6E6E6"))

            # Update axes
            full_plot.update_xaxes(
                gridcolor="#404040",  # Medium gray for grid
                linecolor="#E6E6E6",  # Light gray for axis lines
                tickcolor="#E6E6E6",  # Light gray for ticks
                tickfont=dict(color="#E6E6E6"),  # Light gray for tick labels
                title_font_color="#E6E6E6",  # Light gray for axis title
            )
            full_plot.update_yaxes(
                gridcolor="#404040",  # Medium gray for grid
                linecolor="#E6E6E6",  # Light gray for axis lines
                tickcolor="#E6E6E6",  # Light gray for ticks
                tickfont=dict(color="#E6E6E6"),  # Light gray for tick labels
            ...
            # Make all traces more visible
            for trace in full_plot.data:
                if trace.mode == "lines":
                    if (
                        hasattr(trace.line, "color")
                        and trace.line.color == "black"
                    ):
                        trace.line.color = "#E6E6E6"
                    trace.line.width = 2
        ...
        st.write(
            "### No data to plot. Please reselect in filtered environment dropdown."
        )
